...
1. Open the terminal and perform the following steps:
    ...
    1. Similarly, visit [https://microsoft.com/devicelogin](https://microsoft.com/devicelogin) to complete authentication.
       ...
       {: .note }
	   > This file can be run at the command line or as a script.  It will log in as the SP and then add the listed roles to the SP.  **principalId** is the Enterprise Application Object ID.  To run this script you can save as a bash script with .sh extension , example SPauth.sh and then do a chmod 600 to make it executable.  If you still have problems , you can open a bash prompt in shell.azure.com and run the file at the command line. You will then need to return to VS Code in your browser and **log in as the SP** to run the postprovision.sh below .  This can be done by pasting the first part of the script (up until the *# Service principal block*) above into the command line
